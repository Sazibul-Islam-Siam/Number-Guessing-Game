...
class NumberGuessingGame:
    def __init__(self, master):
        ...
        instruction_label = ttk.Label(input_frame, text="Enter your guess:", background="#eaf6fb", font=("Helvetica", 14, "bold"), foreground="#1976d2")
        ...
        self.attempts_label = ttk.Label(master, text="Attempts: 0", background="#eaf6fb", foreground="#888", font=("Helvetica", 12, "bold"))
        self.attempts_label.pack(pady=(0, 2))

        # Previous guesses section (horizontal)
        prev_frame = ttk.Frame(master)
        prev_frame.pack(pady=(8, 0))
        prev_label = ttk.Label(prev_frame, text="Previous guesses:", background="#eaf6fb", font=("Helvetica", 12, "bold"), foreground="#1976d2")
        prev_label.pack(anchor='w')
        self.guess_canvas = ttk.Frame(prev_frame)
    ...
    def submit_guess(self, event=None):
        try:
            ...
            if guess < self.number_to_guess:
                self.feedback_label.config(text="📉 Too low!", foreground="#e67e22")
                winsound.MessageBeep(winsound.MB_ICONHAND)
                self.progress['value'] = min(guess, 100)
                self.balloon_label.pack_forget()
            elif guess > self.number_to_guess:
                self.feedback_label.config(text="📈 Too high!", foreground="#e67e22")
                winsound.MessageBeep(winsound.MB_ICONHAND)
                self.progress['value'] = min(guess, 100)
                self.balloon_label.pack_forget()
            else:
                self.feedback_label.config(text=f"🎉 Correct! {self.attempts} tries.", foreground="#27ae60")
                winsound.MessageBeep(winsound.MB_ICONASTERISK)
                self.progress['value'] = 100
                self.animate_balloons()
            self.fact_label.config(text=choice(self.facts))
        except ValueError:
            self.feedback_label.config(text="❌ Invalid input! Enter a number.", foreground="#e74c3c")
    ...
    def restart_game(self):
        ...
        self.fact_label.config(text="🔁 New game started. Let's go!")
        self.guess_var.set("")
        self.guess_entry.focus()
